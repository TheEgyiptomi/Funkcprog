...
numberWords' ls = numWordsHelper (words ls) 1
  where
    -- numWordsHelper :: Num a => String -> a -> [(a, String)]
    numWordsHelper [] _ = []
    numWordsHelper (x:xs) n = (n, x) : numWordsHelper xs (n+1)
...
splitAt' n ys@(x:xs)
  | n <= 0 = ([], ys)
  | otherwise = let (e, v) = splitAt' (n - 1) xs in (x : e , v)
...
intersperse' _ [] = []
intersperse' _ [x] = [x]
intersperse' b (x:ys@(y:_)) = x : b : intersperse' b ys
...
unzip' :: [(a, b)] -> ([a], [b])
unzip' [] = ([], [])
unzip' ((a, b):xs) = let (es, vs) = unzip' xs in (a : es, b : vs)
