...
((\x y -> 2 * x + 3 * y) 9) 10 -> (\y -> 2 * 9 + 3 * y) 10 -> 2 * 9 + 3 * 10
...
-- Definiáld a map'' függvényt listagenerátorral!b
...
app2ToFunctions [] = []
app2ToFunctions (f:fs) = f 2 : app2ToFunctions fs
